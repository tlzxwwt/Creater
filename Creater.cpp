...
#define new_game day=1;money=0;fields=1;food=1000;woods=1;wood=1000;houses=0;farmers=0;workers=0;science=0;scientists=0;army=0;armyf=0;treat=0;lighting=0;for(int i=0;i<=maxspells[home];i++){spells[i]=0;}home=1;season=0;percent=1.25;
using namespace std;
...
void help()
{
	...
	fprintf(fi,"春天与秋天，农作物的收获速度增加25\%，但是到了冬天，收获的速度降至5\%\n");
	fprintf(fi,"而升级村庄则可以解锁更多内容！\n");
	fprintf(fi,"一级村庄，冬天的收获速度为20\%，不会有野兽袭击\n");
	fprintf(fi,"二级及以上村庄，冬天的收获速度为5\%，会有野兽袭击\n");
...
inline void task()
{
	if(home<2)
	{
		MessageBox(NULL,"升级你的村庄以解锁此功能","Error",MB_ICONWARNING|MB_OK);
		return;
	}
	qwq
}
